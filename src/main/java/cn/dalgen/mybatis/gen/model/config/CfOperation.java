...
public class CfOperation {
    ...
    private ResultSetTypeEnum resultSetType;
    ...
    public String getFetchSize() {
        if(resultSetType!=ResultSetTypeEnum.DEFAULT){
            String type = ConfigUtil.getCurrentDb().getType();
    ...
    public void validate() {
        //流式读取校验
        if (this.getResultSetType() != ResultSetTypeEnum.DEFAULT) {
            Validate.isTrue(this.getMultiplicity() == MultiplicityEnum.many,
                "流式读取数据要求返回结果必须是many");
            Validate.isTrue(StringUtils.equals(this.getKvMap(), "false"), "流式读取数据不能返回KV");
            Validate.isTrue(this.getMultiplicity() != MultiplicityEnum.paging, "流式读取数据不能是分页");
        }
        //分页校验
        if (this.getMultiplicity() == MultiplicityEnum.paging) {
            Validate.notEmpty(this.getPaging(), "需要设置paging,用来生成分页类");
        }
        if (this.getPagingCntType() == PagingCntTypeEnum.pagingCustom) {
            Validate.notEmpty(this.getPagingCntOperation(),
                "需要设置pagingCntSql,获取分页Operation");
            Validate.notEmpty(this.getPaging(), "需要设置paging,用来生成分页类");
        }

        //kvMap校验
        if (!StringUtils.equals(this.getKvMap(), "false")) {
            Validate.isTrue(this.getMultiplicity() == MultiplicityEnum.many,
                "转KvMap要求返回结果必须是many");
            Validate.isTrue(StringUtils.isNotBlank(this.getMapK()), "转KvMap要求必填mapK");
        }
    }
}
